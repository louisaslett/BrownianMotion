...
#'
#' @export
sim.condbblocal <- function(bm, s, t, q = NULL, q.grid = NULL, label = names(q)) {
  # Arg types & combos
  ...
  for(qq in q) {
    bm.res <- sim.condbblocal_(bm, s_idx, qq, t_idx, label[qq==q])
    s_idx <- s_idx+1
    t_idx <- t_idx+1
  }
...
sim.condbblocal_ <- function(bm, s_idx, q, t_idx, label) {
  ...
  if(soft) {
    sim.condlocalsoft_(bm$bb.local, bb.s_idx, q, bb.t_idx, label)
  } else {
    sim.condlocal_(bm$bb.local, bb.s_idx, q, bb.t_idx, label)
  }
