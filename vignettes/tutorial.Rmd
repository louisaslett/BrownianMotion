...
bm[t = 1, "W_t"]
```
Note that by default calls to the `bm` environment without specifying the `t` argument will instead assume the user wants the associated index. For instance:
...
Of course, we may also wish to know what value(s) the path took at time(s) given by a specific label. This can be achievied by simply passing a label (for instance, `special.time.1`) as the index in a call to `bm`:
...
For many practical uses of the package, the user may wish to add in path discontinuities. In such settings we will have a number of path "segments" (the term we give for the continuous paths separated by the path discontinuities). To aid this, in addition to `$t` and `$W_t`, the stored path actually contains `$W_tm`:
...
We can access the vector associated with `W_tm` by calling the environment in the same way as previously. Alternatively, by not specifying any argument then calling the enviroment returns `t`, `W_t`, `W_tm` and `layers`:
