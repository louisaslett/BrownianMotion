...
[NOTE:] Following this logic, we should be able to toggle off the outer solid lines by using `plot(bm, hide.user=TRUE)`
...
then if we want to simulate the path at $t=2$ and have resultant Bessel layers then this needs to be specified by the user as it defers from the default on the paths creation (`prefer = "intersection"`):
...
In particular, further simulation of the path at $t=1.5$ and $t=2.5$ results in both Bessel layers (in the interval $[1,2]$ and $[2,3]$ being converted to Intersection layers: 
...
results in any Bessel layers being converted to Intersection layers. Similary we could attempt to convert back to Bessel layers (although this may not always be possible for every interval): 
...
bm4 %>% sim(seq(0,10,length=20))
plot(bm4)
```

Whether the package does refinement by default is inherited from the attribute when the path is created. We can either change this at some future point globally (for instance, `bm3$refine <- TRUE`), or alternatively specify within the other functionals in the same manner as the `prefer` attribute discussed earlier. For instance, consider the introduction of a single additional point at $t=\pi$:

```{r}
bm3 %>% sim(pi,prefer = TRUE)
plot(bm3)
```

Note that the intervals surrounding $t=\pi$ have now been refined.
...
```{r}
bm5 %>% sim(seq(0,10,length=20))
plot(bm5)
```

If we introduce an additional point (say at $t=pi$), but we want increased increased refinement (say of `mult=1`) this can be done by changing the default argument in any of the earlier functions as before. For instance: 
...
```{r}
bm5 %>% refine(5,10,mult = 0.25)
plot(bm5)
```

The `mult = 0.25` argument has increased the resolution of existing layers in the interval $[5,10]$ by $3/0.25=12$x.
